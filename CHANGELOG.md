...
First release from the `AndreKurait/docker-cache` fork. Source repository
moved from `ScribeMD/docker-cache` to `AndreKurait/docker-cache`.

### Feat

- **runtime**: Upgrade GitHub Actions runtime from `node20` to `node24` (#49, #51)
- **windows**: Handle Docker startup races on Windows runners (#30)
- **windows**: Add support for `windows-2025` runners alongside `windows-latest` (#11)
- **linux**: Add support for `ubuntu-24.04` runners (#33, #8)

### Fix

- **deps**: Bump `@actions/cache` from v4.0.3 to v6.0.1 (#1)
- **deps**: Bump `@actions/core` from v1.11.1 to v3.0.1 (#2)
- **restore-cache**: Harden image verification and cache cleanup; remove
  platform-dependent dangling-image assertion (#10, #12)
- **build**: Exclude test sources from action bundle compilation so
  `yarn build` produces a `dist/` consistent with `package.json`.

### Chore

- Numerous dev-dependency, lock-file, and pre-commit hook bumps. See git log
  for the full list.
